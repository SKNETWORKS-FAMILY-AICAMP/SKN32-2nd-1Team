...
# 로그인 후, "개인별 이탈 예측" 메뉴일 때 고객 이탈 예측 서비스를 표시합니다.
    ...
    provider_opts = code_options(range(1, 5), PROVIDER_LABELS)
    cost_payer_opts = code_options(range(1, 7), COST_PAYER_LABELS)
    bundled_opts = code_options(range(0, 2), BUNDLED_LABELS)
    ...
    with st.form("churn_form"):
        ...
        with col2:
            ...
            income = st.number_input("월평균 소득 (income, 만원)", min_value=0, max_value=2000, value=300, step=10)
            area_kr = st.selectbox("지역 (area)", list(area_opts.keys()))
            household_size = st.number_input("가구원 수 (household_size)", min_value=1, max_value=10, value=3, step=1)
